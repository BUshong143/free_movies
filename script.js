...
const PH_GENRES=[
  {id:28,n:'Action'},{id:35,n:'Comedy'},{id:18,n:'Drama'},
  {id:27,n:'Horror'},{id:10749,n:'Romance'},{id:10751,n:'Family'}
];
// Philippines region code for TMDB = PH, language = tl or fil
const PH_REGION='PH';
const PH_LANG='tl';
 
let heroItems=[],heroIdx=0,heroTimer,curPage='home';
let pId,pType,pTitle;
const booted={home:false,anime:false,tv:false,movies:false,ph:false};
 
/* ──────────────────────────────────
   SOURCES — Clean, ad-reduced embeds
   The sandbox on the iframe handles popup blocking.
   We pick sources known for fewer intrusive ads first.
────────────────────────────────── */
function getSrc(id, type){
  const t = type==='tv'?'tv':'movie';
  return [
    {n:'VidSrc·Pro',  u:`https://vidsrc.pro/embed/${t}/${id}`},
    {n:'VidSrc·xyz',  u:`https://vidsrc.xyz/embed/${t}/${id}`},
...
async function getAnimeEmbedSrcs(malId, title){
  try{
    const r=await tmdb('/search/tv',`&query=${encodeURIComponent(title)}`);
    if(r.results?.[0]) return getSrc(r.results[0].id,'tv');
  }catch(e){}
  try{
    const r=await tmdb('/search/movie',`&query=${encodeURIComponent(title)}`);
    if(r.results?.[0]) return getSrc(r.results[0].id,'movie');
  }catch(e){}
  const enc=encodeURIComponent(title);
  return [
    {n:'VidSrc·Pro', u:`https://vidsrc.pro/embed/tv/${malId}`},
    {n:'VidSrc·xyz', u:`https://vidsrc.xyz/embed/tv/${malId}`},
    {n:'Animepahe',  u:`https://animepahe.ru/search?q=${enc}`},
    {n:'HiAnime',    u:`https://hianime.to/search?keyword=${enc}`},
  ];
}
...
/* ── RENDER ── */
function renderTMDB(arr,id,isTV=false,isPH=false){
  const el=document.getElementById(id);if(!el)return;
  if(!arr||!arr.length){el.innerHTML='<p style="color:var(--muted);padding:20px;font-size:12px">No results found</p>';return;}
  el.innerHTML=arr.map(m=>{
    ...
    const img=m.poster_path?`<img src="${IBASE}w342${m.poster_path}" alt="${esc(t)}" decoding="async" loading="lazy">`:`<div class="card-np">🎬</div>`;
    const tp=isTV?'tv':'movie';
    const badge=isPH?`<span class="ph-badge">PH</span>`:'';
    return `<div class="card" onclick="openDetail(${m.id},'${tp}')">
      ...
      <div class="card-info">
        ...
        <div class="card-sub"><span>${y}</span>${badge}</div>
      </div>
    </div>`;
  }).join('');
}
...
function renderAnime(arr,id){
  ...
  el.innerHTML=arr.map(a=>{
    ...
    const poster=imgSrc?`<img src="${imgSrc}" alt="${esc(t)}" decoding="async" loading="lazy">`:`<div class="card-np">⛩️</div>`;
...
async function bootHome(){
  ['r-trending','r-toprated','r-nowplaying','r-tvpop','r-anime','r-pinoy'].forEach(i=>document.getElementById(i).innerHTML=sk());
...
async function loadPinoyPreview(){
  try{
    const d=await tmdb('/discover/movie',`&with_original_language=tl&sort_by=popularity.desc&vote_count.gte=5`);
    renderTMDB(d.results||[],'r-pinoy',false,true);
  }catch(e){document.getElementById('r-pinoy').innerHTML='<p style="color:var(--muted);padding:20px;font-size:12px">Loading Pinoy films…</p>';}
}
...
/* ── BOOT PINOY ── */
async function bootPH(){
  ['ph-trending','ph-toprated','ph-action','ph-romance','ph-comedy','ph-horror'].forEach(i=>document.getElementById(i).innerHTML=sk());
  // Genre pills for PH page
  document.getElementById('ph-pills').innerHTML=PH_GENRES.map(g=>`<div class="g-pill ph" data-gid="${g.id}" onclick="phGenre(${g.id},'${g.n}')">${g.n}</div>`).join('');
  try{
    const[trend,top,action,romance,comedy,horror]=await Promise.all([
      // Trending PH films by popularity
      tmdb('/discover/movie',`&with_original_language=tl&sort_by=popularity.desc&vote_count.gte=5`),
      // Top rated
      tmdb('/discover/movie',`&with_original_language=tl&sort_by=vote_average.desc&vote_count.gte=10`),
      // Action
      tmdb('/discover/movie',`&with_original_language=tl&with_genres=28&sort_by=popularity.desc`),
      // Romance + Drama
      tmdb('/discover/movie',`&with_original_language=tl&with_genres=10749&sort_by=popularity.desc`),
      // Comedy
      tmdb('/discover/movie',`&with_original_language=tl&with_genres=35&sort_by=popularity.desc`),
      // Horror
      tmdb('/discover/movie',`&with_original_language=tl&with_genres=27&sort_by=popularity.desc`),
    ]);
    renderTMDB(trend.results,'ph-trending',false,true);
    renderTMDB(top.results,'ph-toprated',false,true);
    renderTMDB(action.results,'ph-action',false,true);
    renderTMDB(romance.results,'ph-romance',false,true);
    renderTMDB(comedy.results,'ph-comedy',false,true);
    renderTMDB(horror.results,'ph-horror',false,true);
  }catch(e){
    ['ph-trending','ph-toprated','ph-action','ph-romance','ph-comedy','ph-horror'].forEach(i=>{
      document.getElementById(i).innerHTML='<p style="color:var(--muted);padding:20px;font-size:12px">Failed to load</p>';
    });
  }
}
 
async function phGenre(gid, name){
  switchPage('ph');
  document.querySelectorAll('.g-pill.ph').forEach(p=>p.classList.toggle('active',+p.dataset.gid===gid));
  document.getElementById('ph-genre-sec').style.display='block';
  document.getElementById('ph-genre-title').innerHTML=`<span class="sec-num ph">+</span>${name} — Filipino`;
  document.getElementById('ph-genre-car').innerHTML=sk();
  try{
    const d=await tmdb('/discover/movie',`&with_original_language=tl&with_genres=${gid}&sort_by=popularity.desc`);
    renderTMDB(d.results,'ph-genre-car',false,true);
  }catch(e){document.getElementById('ph-genre-car').innerHTML='<p style="color:var(--muted);padding:20px;font-size:12px">Failed to load</p>';}
  document.getElementById('ph-genre-sec').scrollIntoView({behavior:'smooth',block:'start'});
}
...
async function switchPage(pg){
  ...
  document.querySelectorAll('.sb-item[data-sid]').forEach(t=>{
    t.classList.remove('active','ph-active');
    if(t.dataset.sid===pg) t.classList.add(pg==='ph'?'ph-active':'active');
  });
  ...
  if(pg==='ph'&&!booted.ph){booted.ph=true;await bootPH();}
...
async function openDetail(id,type){
  ...
  try{
    ...
    document.getElementById('mmeta').innerHTML=`
      ...
      ${d.original_language==='tl'?`<span class="tag" style="color:var(--ph);border-color:rgba(232,55,90,.3)">🇵🇭 Filipino</span>`:''}`;
    const enc=encodeURIComponent(title);
...
async function launchAnime(malId, rawEncTitle){
  ...
  buildSourceTabs(srcs, 'loadAnimeSrc');
  document.getElementById('player').classList.add('open');
  document.body.style.overflow='hidden';
  loadAnimeSrc(0);
}
...
function buildSourceTabs(srcs, fnName){
  document.getElementById('stabs').innerHTML=srcs.map((s,i)=>`
    <button class="s-tab ${i===0?'active':''}" onclick="${fnName}(${i})">${s.n}</button>
  `).join('');
}
 
function setActiveTab(i){
  document.querySelectorAll('#stabs .s-tab').forEach((b,j)=>b.classList.toggle('active',j===i));
}
 
function loadFrame(url){
  const ld=document.getElementById('pload');
  ld.classList.remove('gone');
  document.getElementById('otab').href=url;
  const fr=document.getElementById('pFrame');
  fr.src='';
  setTimeout(()=>{
    fr.src=url;
    fr.onload=()=>ld.classList.add('gone');
    setTimeout(()=>ld.classList.add('gone'),10000);
  },80);
}
...
async function doSearch(q){
  ...
  const tmdbCards=tmdbRes.map(m=>{
    ...
    const phBadge=m.original_language==='tl'?`<span class="ph-badge">PH</span>`:'';
    return `<div class="card" onclick="openDetail(${m.id},'${tp}')">
      <div class="card-poster">${img}<div class="card-rating-badge">⭐${r}</div>
        <div class="card-overlay"><div class="card-play">▶</div><div class="card-ov-title">${esc(t)}</div></div>
      </div>
      <div class="card-info"><div class="card-title">${esc(t)}</div><div class="card-sub"><span>${y}</span>${phBadge}</div></div>
    </div>`;
  });
